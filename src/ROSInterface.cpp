...
ROSInterface::ROSInterface(const boost::property_tree::ptree& config)
  ...
  askincremental_next_solution_action_server_ =
      rclcpp_action::create_server<AskIncrementalNext>(
          this, "knowrob/askincremental_next_solution",
          std::bind(&ROSInterface::handle_goal_askincremental_next, this, _1,
                    _2),
          std::bind(&ROSInterface::handle_cancel_askincremental_next, this, _1),
          std::bind(&ROSInterface::handle_accepted_askincremental_next, this,
                    _1));
  ...
  ask_incremental_finish_srv_ = this->create_service<srv::AskIncrementalFinish>(
      ...
      std::bind(&ROSInterface::handle_ask_incremental_finish, this, _1, _2,
                _3));
...
ROSInterface::translateModalityFrameMessage(const msg::ModalFrame& frame) {
...
msg::GraphAnswerMessage ROSInterface::createGraphAnswer(
    std::shared_ptr<const AnswerYes> answer) {
...
// ----------------- AskAll -----------------
rclcpp_action::GoalResponse ROSInterface::handle_goal_askall(
    const rclcpp_action::GoalUUID&, std::shared_ptr<const AskAll::Goal>) {
  RCLCPP_INFO(get_logger(), "AskAll goal received");
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse ROSInterface::handle_cancel_askall(
    const std::shared_ptr<GoalHandleAskAll>) {
  RCLCPP_INFO(get_logger(), "AskAll goal canceled");
  return rclcpp_action::CancelResponse::ACCEPT;
}

void ROSInterface::handle_accepted_askall(
...
void ROSInterface::execute_askall(
    const std::shared_ptr<GoalHandleAskAll> goal_handle) {
  ...
  result->status = (count > 0) ? AskAll::Result::TRUE : AskAll::Result::FALSE;

  goal_handle->succeed(result);
}

// ----------------- AskOne -----------------
rclcpp_action::GoalResponse ROSInterface::handle_goal_askone(
    const rclcpp_action::GoalUUID&, std::shared_ptr<const AskOne::Goal>) {
  RCLCPP_INFO(get_logger(), "AskOne goal received");
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse ROSInterface::handle_cancel_askone(
    const std::shared_ptr<GoalHandleAskOne>) {
  RCLCPP_INFO(get_logger(), "AskOne goal canceled");
  return rclcpp_action::CancelResponse::ACCEPT;
}

void ROSInterface::handle_accepted_askone(
    const std::shared_ptr<GoalHandleAskOne> goal_handle) {
  std::thread(&ROSInterface::execute_askone, this, goal_handle).detach();
}

void ROSInterface::execute_askone(
    const std::shared_ptr<GoalHandleAskOne> goal_handle) {
  auto goal = goal_handle->get_goal();
  FormulaPtr phi = QueryParser::parse(goal->query.query_string);
  FormulaPtr mPhi = InterfaceUtils::applyModality(
...
// ----------------- AskIncremental -----------------
rclcpp_action::GoalResponse ROSInterface::handle_goal_askincremental(
...
rclcpp_action::CancelResponse ROSInterface::handle_cancel_askincremental(
    const std::shared_ptr<GoalHandleAskIncremental>) {
  RCLCPP_INFO(get_logger(), "AskIncremental canceled");
  return rclcpp_action::CancelResponse::ACCEPT;
}

void ROSInterface::handle_accepted_askincremental(
    const std::shared_ptr<GoalHandleAskIncremental> goal_handle) {
  std::thread(&ROSInterface::execute_askincremental, this, goal_handle)
      .detach();
}

void ROSInterface::execute_askincremental(
    const std::shared_ptr<GoalHandleAskIncremental> goal_handle) {
  auto goal = goal_handle->get_goal();
  FormulaPtr phi = QueryParser::parse(goal->query.query_string);
  FormulaPtr mPhi = InterfaceUtils::applyModality(
...
// ----------------- AskIncrementalNextSolution -----------------
rclcpp_action::GoalResponse ROSInterface::handle_goal_askincremental_next(
    const rclcpp_action::GoalUUID&,
...
rclcpp_action::CancelResponse ROSInterface::handle_cancel_askincremental_next(
    const std::shared_ptr<GoalHandleAskIncrementalNext>) {
  return rclcpp_action::CancelResponse::ACCEPT;
}

void ROSInterface::handle_accepted_askincremental_next(
    const std::shared_ptr<GoalHandleAskIncrementalNext> goal_handle) {
  std::thread(&ROSInterface::execute_askincremental_next, this, goal_handle)
      .detach();
}

void ROSInterface::execute_askincremental_next(
    const std::shared_ptr<GoalHandleAskIncrementalNext> goal_handle) {
  auto goal = goal_handle->get_goal();
...
// ----------------- Tell -----------------
rclcpp_action::GoalResponse ROSInterface::handle_goal_tell(
    const rclcpp_action::GoalUUID&, std::shared_ptr<const Tell::Goal>) {
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse ROSInterface::handle_cancel_tell(
    const std::shared_ptr<GoalHandleTell>) {
  return rclcpp_action::CancelResponse::ACCEPT;
}

void ROSInterface::handle_accepted_tell(
    const std::shared_ptr<GoalHandleTell> goal_handle) {
  std::thread(&ROSInterface::execute_tell, this, goal_handle).detach();
}

void ROSInterface::execute_tell(
    const std::shared_ptr<GoalHandleTell> goal_handle) {
  auto goal = goal_handle->get_goal();
  ...
  result->status = ok ? Tell::Result::TRUE : Tell::Result::TELL_FAILED;
  feedback->finished = true;
  goal_handle->publish_feedback(feedback);
  goal_handle->succeed(result);
}
...
// ----------------- Services -----------------
void ROSInterface::handle_ask_incremental_finish(
...
void ROSInterface::handle_export_triples(
    const std::shared_ptr<rmw_request_id_t>,
  ...
  if (req->format == "rdfxml") {
    ...
    RCLCPP_ERROR(get_logger(), "Unsupported export format: %s",
                 req->format.c_str());
    res->success = false;
  }
}
