...
class ROSInterface : public rclcpp::Node {
 ...
 private:
  ...
  using GoalHandleAskIncremental =
      rclcpp_action::ServerGoalHandle<AskIncremental>;
  ...
  std::unordered_map<std::string, boost::any> translateModalityFrameMessage(
      const msg::ModalFrame& frame);

  msg::GraphAnswerMessage createGraphAnswer(
      std::shared_ptr<const knowrob::AnswerYes> answer);
  ...
  std::shared_ptr<rclcpp_action::Server<AskIncremental>>
      askincremental_action_server_;
  std::shared_ptr<rclcpp_action::Server<AskIncrementalNext>>
      askincremental_next_solution_action_server_;
  std::shared_ptr<rclcpp_action::Server<Tell>> tell_action_server_;

  // —— Services ——————————————————————————————————————
  rclcpp::Service<srv::AskIncrementalFinish>::SharedPtr
      ask_incremental_finish_srv_;
  rclcpp::Service<srv::ExportTriples>::SharedPtr export_srv_;

  // —— Query state ————————————————————————————————
  std::mutex query_mutex_;
  std::map<uint32_t, std::shared_ptr<knowrob::TokenQueue>> query_results_;
  ...
  void handle_accepted_askall(const std::shared_ptr<GoalHandleAskAll>);
  void execute_askall(const std::shared_ptr<GoalHandleAskAll>);
  ...
  void handle_accepted_askone(const std::shared_ptr<GoalHandleAskOne>);
  void execute_askone(const std::shared_ptr<GoalHandleAskOne>);
  ...
  void handle_accepted_askincremental(
      const std::shared_ptr<GoalHandleAskIncremental>);
  void execute_askincremental(const std::shared_ptr<GoalHandleAskIncremental>);
  ...
  void handle_accepted_askincremental_next(
      const std::shared_ptr<GoalHandleAskIncrementalNext>);
  void execute_askincremental_next(
      const std::shared_ptr<GoalHandleAskIncrementalNext>);
  ...
  void handle_accepted_tell(const std::shared_ptr<GoalHandleTell>);
  void execute_tell(const std::shared_ptr<GoalHandleTell>);
  ...
  void handle_export_triples(const std::shared_ptr<rmw_request_id_t>,
